...
【タイトル】
（40文字ちょうどになるまで使い切ること。文章にせず、検索されやすい単語を並べる形にする。アイテム名、サイズ、色、ブランド名（英語表記とカタカナ表記の両方が一般的なら両方）、素材、系統・デザインの特徴を、優先度が高い順に並べて40文字に収まるだけ詰め込む。優先度が低い単語（素材・系統・デザインの特徴）から先に削って調整すること。ただし系統・雰囲気を表す単語は、複数思いついても合計1〜2個までに絞ること。使う場合は上の【当店で使っている系統一覧】の中から商品に最も近いものを選び、一覧に無い独自の雰囲気ワードを何個も並べないこと）
